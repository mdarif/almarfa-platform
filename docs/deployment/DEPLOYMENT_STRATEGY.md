...
- Both `SITE_URL` and `CF_PAGES_URL` are declared in `turbo.json` `globalEnv` so Turborepo invalidates the build cache whenever they change — without this, a stale cached build would silently serve incorrect canonicals after a domain switch
...
- **Custom domain (later):** set `SITE_URL=https://almarfa.co` in Cloudflare Pages **production** environment only when `almarfa.co` is connected
